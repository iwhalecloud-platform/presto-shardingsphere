 ...
 */
package com.iwhalecloud.zdaas.presto.shardingsphere.sql;
 ...
 */
public class ShardingSphereSqlBuilder
{
    ...
    public ShardingSphereSqlBuilder(String quote)
    {
        this.quote = requireNonNull(quote, "quote is null");
    }


    public ShardingSphereSqlBuilder withLimit(int limit) {
        this.limit = limit;
        return this;
    }
            ...
            throws SQLException
    {
        ...
        final String targetSql = sql.toString();

        PreparedStatement statement = client.getPreparedStatement(connection, targetSql);
    ...
    private static boolean isAcceptedType(Type type)
    {
        ...
        if (validType instanceof DecimalType) {
            return ((DecimalType) validType).getScale() == 0;
        }
